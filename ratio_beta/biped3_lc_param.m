...
for j = 1 : n-1 
    ...
    sys = struct('g',9.81,'k',17.8389,'E',1.0398,'v',0,'theta',0,'th0',th0_per(j),'beta',beta_per(j),...
                'ratio',ratio_per(j),'d',0,'w',0,'theta_abs',0,'beta_abs',0) ;
    ...
    pattern_in = 0 ;
    for k = 1 : j
       if (abs(theta_abs(k) - sys.theta_abs) < 1e-5 && abs(beta_abs(k) - sys.beta_abs) < 1e-5 ...
           && abs(ratio(k) - sys.ratio) < 1e-5 && abs(beta(k) - sys.beta) < 1e-5 )
...
save('ratio_beta_lc','q_init','dl','wl','dr','wr','t_sl','t_sr','ratio','beta','theta_abs','beta_abs','v_fwd','v_lat','v_lat_TD','v_fwd_TD')
