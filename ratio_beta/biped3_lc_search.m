...
sys = struct('g',9.81,'k',0,'E',0,'v',0,'theta',0,'th0',0,'beta',0,'d',0,'w',0,'ratio',0,'xcom_marg',0,'rot',0) ;
...
ratio_per = zeros(1,500) ; % ratio between velocity and gravity vectors. 
                           % Used in the calculation of frontal (or polar)
                           % leg angle theta in biped3_step.
...
for th0 = 84 : 0.2 : 88
    ...
    % Range of ratio for theta angle
    for ratio = 0 : 0.02 : 0.9
        
        sys.ratio = ratio ;
        
        % Range of lateral angle beta
        for beta = 0.03 : 0.01 : 0.12
            ...
            if (~fall_chk && max(abs(eigenv)) <= 1)
                ...
                fprintf('Limit cycle found for th0 = %f , beta = %f, ratio = %f \n',[th0, beta, ratio])
            end
        end
    end
    fprintf('done \n')
...
% save('ratio_beta','s_per','eig_per','beta_per','ratio_per','th0_per')
% exit
