...
function [step, fall_chk, sys] = biped3_step(q0, i_step, sys)
    ...
    if ie == 1 % check if single support was successful without violation of any of the falling conditions in biped3_TD
        ...
        sys.beta_abs = sys.beta + abs(ang_v_lat) ; % Absolute value of lateral leg angle as measured from the saggital plane
        ...
        if mod(i_step,2) == 0
            sys.w = q_ss(end,3) + cos(-sys.theta) * sin(-sys.beta + ang_v_lat) ;
            sys.d = q_ss(end,1) + cos(-sys.theta) * cos(-sys.beta + ang_v_lat) ;
