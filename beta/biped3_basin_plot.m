...
clear
load('beta_basin_all.mat') % Change this to ratio_beta_basin_ex.mat for plotting the basin of a single pattern
load('beta_lc.mat')

count = zeros(size(beta_basin,3),1) ;
bound = zeros(30, size(beta_basin,3)) ;
bound_sz = zeros(size(beta_basin,3),1) ;

for k = 1:size(beta_basin,3)
    for i = 1:size(beta_basin,1)
        
        if beta_basin(i,1,k) ~= 0
            count(k) = count(k) + 1 ;
        end
        
    end
    k_bound = boundary(beta_basin(1:count(k),:,k),0) ;
...
col = zeros(size(beta_basin,3),3) ;
area = zeros(size(beta_basin,3),1) ;
...
for k = 1:size(beta_basin,3)
    ...
    y = beta_basin(bound(1:bound_sz(k),k),1,k) ; 
    z = beta_basin(bound(1:bound_sz(k),k),2,k) ; 
...
%title('Basin of attraction of patterns of beta strategy')
grid on
